...

{
  ...
  "core_insight": "The single most important thing the framework sees in this query. One paragraph. Framework language is now permitted — but only after recognition has landed. Aphoristic compression preferred over exhaustive explanation. This is the encounter's center of gravity. Close by locating the specific question this insight itself now raises for the person's own life — using the same technique recognition used on the original query: find what is underneath, not what is generic. This is the load-bearing part of the response, not a sign-off — the point where the framework stops speaking and hands the next move to the person. It must be answerable only given what was just actually said, not swappable onto a different response. Wrong: 'Want to go there?' 'Worth exploring further?' Right: 'What does that moment feel like for you?' 'If the God who authors calamity is also the God who entered it, what does that ask of you right now?' This question belongs here, inside core_insight, where it always renders. This is distinct from thread_summary's closure rule (ends solid, not with a question) — that governs winding down an extended thread across many exchanges; this governs every individual full_spectrum response, including the first.",
