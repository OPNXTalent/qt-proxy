...
export function validateAnalysis(candidate) {
  ...
  const learning = candidate?.learningCandidate;
  return {
    reduction: {
    ...
    learningCandidate: learning && typeof learning === 'object' && !Array.isArray(learning)
      ? {
          eligible: learning.eligible === true,
          deidentified: learning.deidentified === true,
          topic: cleanText(learning.topic, 160),
          lesson: cleanText(learning.lesson, 1200),
          rationale: cleanText(learning.rationale, 800),
          applicability: cleanText(learning.applicability, 500),
          boundaries: cleanText(learning.boundaries, 500),
          tags: cleanStringList(learning.tags, 8, 48),
        }
      : null,
  };
}
...
export function validateAnalysisStrict(candidate, rawText = '') {
  ...
  const rawFalsifiabilityStatus = candidate.constraintGate?.falsifiabilityStatus;
  const normalizedFalsifiabilityKey = typeof rawFalsifiabilityStatus === 'string'
    ? rawFalsifiabilityStatus.trim().toLowerCase().replace(/[\s-]+/g, '_')
    : rawFalsifiabilityStatus;
  const falsifiabilityAliases = {
    falsifiable: 'testable',
    partially_falsifiable: 'partially_testable',
    non_falsifiable: 'unfalsifiable',
    not_testable: 'unfalsifiable',
    none: 'not_applicable',
    n_a: 'not_applicable',
    not_relevant: 'not_applicable',
  };
  const normalizedFalsifiabilityStatus = falsifiabilityAliases[normalizedFalsifiabilityKey]
    || normalizedFalsifiabilityKey;
  const rawEndingMode = candidate.constraintGate?.endingMode;
  const normalizedEndingKey = typeof rawEndingMode === 'string'
    ? rawEndingMode.trim().toLowerCase().replace(/[\s-]+/g, '_')
    : rawEndingMode;
  const endingModeAliases = {
    declarative_closure: 'declarative',
    statement: 'declarative',
    conclusion: 'declarative',
    interrogative_closure: 'interrogative',
    question: 'interrogative',
  };
  const aliasedEndingMode = endingModeAliases[normalizedEndingKey]
    || normalizedEndingKey;
  // questionNeeded carries the semantic decision. Preserve the strict
  // declarative/interrogative contract without making provider wording part
  // of the runtime's epistemic identity.
  const normalizedEndingMode = ['declarative', 'interrogative'].includes(aliasedEndingMode)
    ? aliasedEndingMode
    : typeof candidate.constraintGate?.questionNeeded === 'boolean'
      ? candidate.constraintGate.questionNeeded ? 'interrogative' : 'declarative'
      : aliasedEndingMode;
  const constraintAliases = {
    ontology: 'ontological',
    ontological_constraint: 'ontological',
    being: 'ontological',
    logic: 'logical',
    logical_constraint: 'logical',
    reason: 'logical',
    phenomenology: 'phenomenological',
    phenomenological_constraint: 'phenomenological',
    empirical: 'phenomenological',
    experiential: 'phenomenological',
    experience: 'phenomenological',
    relation: 'relational',
    relational_constraint: 'relational',
    relational_coherence: 'relational',
    coherence: 'relational',
  };
  ...
  const rawRelevantConstraints = candidate.constraintGate?.relevantConstraints;
  const normalizedRelevantConstraints = Array.isArray(rawRelevantConstraints)
    ? Array.from(new Set(rawRelevantConstraints.map((value) => {
      if (typeof value !== 'string') return null;
      const key = value.trim().toLowerCase().replace(/[\s-]+/g, '_');
      const normalized = constraintAliases[key] || key;
      return allowedConstraints.has(normalized) ? normalized : null;
    }).filter(Boolean))).slice(0, 4)
    : rawRelevantConstraints;
  if (Array.isArray(normalizedRelevantConstraints)
    && normalizedRelevantConstraints.length === 0) {
    normalizedRelevantConstraints.push('logical');
  }
  const normalizedCandidate = {
    ...candidate,
    constraintGate: {
      ...candidate.constraintGate,
      falsifiabilityStatus: normalizedFalsifiabilityStatus,
      relevantConstraints: normalizedRelevantConstraints,
      endingMode: normalizedEndingMode,
    },
  };
  const reducer = normalizedCandidate.reduction;
  const gate = normalizedCandidate.constraintGate;
  ...
  if (typeof normalizedCandidate.structuralDelta !== 'string'
    || !normalizedCandidate.structuralDelta.trim()) {
  ...
  const constraintValues = allowedConstraints;
  ...
  if (!Array.isArray(normalizedCandidate.changedStateFields)) {
    throw new Error('CHANGED_STATE_FIELDS_INVALID');
  }
  const patch = normalizedCandidate.statePatch;
  ...
  // Reducers occasionally echo fields from PREVIOUS INQUIRY STATE even though
  // the prompt asks for a delta. These fields are owned by the server and are
  // derived during applyInquiryPatch; discard them rather than failing an
  // otherwise valid user turn. Continue to reject genuinely unknown fields.
  const serverManagedPatchFields = new Set([
    'schemaVersion',
    'version',
    'turn',
  ...
  const unknownPatchFields = Object.keys(patch)
    .filter(field => !allowedPatchFields.has(field) && !serverManagedPatchFields.has(field));
  if (unknownPatchFields.length) {
    throw new Error('STATE_PATCH_FIELD_INVALID');
  }
  const canonicalPatch = Object.fromEntries(
    Object.entries(patch).filter(([field]) => allowedPatchFields.has(field)),
  );
  const patchFields = Object.keys(canonicalPatch);
  // statePatch is the authoritative, allowlisted delta. changedStateFields is
  // explanatory metadata and must not become a second, conflicting source of
  // truth when a model omits an otherwise valid patch key from that list.
  const canonicalChangedStateFields = [...new Set([
    ...normalizedCandidate.changedStateFields,
    ...patchFields.filter(field => field !== 'trajectoryChange'),
  ])];
  const validated = validateAnalysis({
    ...normalizedCandidate,
    changedStateFields: canonicalChangedStateFields,
    statePatch: canonicalPatch,
  });
...
export function buildReducerPrompt({ state, input, userCorrection = false, learningAllowed = true }) {
...
LEARNING ELIGIBILITY: ${learningAllowed ? 'ALLOWED — apply the learningCandidate rules below.' : 'EXCLUDED — this is a private or collaborative exchange. Set learningCandidate.eligible to false and do not distill a lesson.'}
...
3. Apply a lightweight gate: observations versus inferences, evidence boundaries, unsupported assumptions, and unfalsifiable claims.
   relevantConstraints may contain only ontological, logical, phenomenological, and relational. Use logical if uncertain.
...
7. falsifiabilityStatus must be exactly one of: testable, partially_testable, unfalsifiable, not_applicable.
8. changedStateFields should name every statePatch key except trajectoryChange; statePatch remains the authoritative delta.
9. When learning eligibility is ALLOWED, evaluate whether the exchange produced a durable, generally applicable lesson that could improve The Prism. Favor explicit user corrections, demonstrated reasoning improvements, and reusable presentation constraints. Do not treat a user's religious belief, identity, personal history, preference, or mere disagreement as a lesson.
10. A learningCandidate is eligible only when it is fully de-identified and can stand alone without the conversation. Never include names, emails, account or thread identifiers, quotations, personal events, inferred religious affiliation, private facts, or details from Private Notes, Circle of Trust, or other shared/collaborative exchanges. State the lesson as a product-level reasoning or response principle, not as a claim about the user.
...
export function detectExplicitCorrection(input) {
  return /\b(?:no[,—-]|that(?:'s| is) not what i (?:said|meant)|you (?:misread|misunderstood)|let me (?:correct|clarify)|i mean|rather|this (?:part|section|analogy|comparison) (?:is|was)|doesn'?t add|does not add|omit(?:ting)?|remove|avoid|don'?t use|do not use|filler|foder|overreach)\b/i
    .test(input || '');
}
