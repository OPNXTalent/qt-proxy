...
//
// Visibility note: unlike Refractions (share_chat_messages, handled in a
// separate file), the main Q&A chain has NO per-entry private/public split.
// Once a thread is Trust Circle, every follow-up in it is visible to the
// whole circle by default — hiding an actual interpretive answer was
// confirmed out of scope for this table.
//
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (shareId) {
      ...
      if (shares?.length) {
        ...
        // Anonymous viewer has no mute list of their own — sees everything.
  ...
  if (req.method === 'POST') {
    ...
    // A genuine new interpretive follow-up, exactly like the owner's own
    // main-view flow — billed to the PARTICIPANT's own account via
    // /api/interpret before this call; this is just persistence. Requires
    // the caller to already be an ACTIVE participant (joined via the "add
    // to my Archive" action) — simply knowing a thread's ID isn't enough.
    ...
    // ── Owner contribution — unchanged from before ───────────────────────────
    if (!userEmail) return res.status(401).json({ error: 'Unauthorized' });
