...
function shouldLoadTheodicyModule(query, inquiryClassification) {
  ...
  const challengePhrases = [
    ...
    /how can god (judge|condemn|punish).*(created|made|knew|foreknew|omniscient)/i,
    /god.*(created|made).*(knowing|knew|foreknew).*(sin|evil|reject|condemn|damn|destruction)/i,
    /(vessels? of|vessel).*(honor|honour|dishonor|dishonour|wrath|destruction)/i,
    /(universalism|all (will|shall|must) be saved).*(scripture|biblical|bible|judgment|condemn|destruction|narrow|broad)/i,
...
const cases = [
  ...
  { q: 'How can God judge someone He created knowing every evil choice they would make?',
    cls: 'Theological',  expected: LOAD,    note: 'Foreknowledge and judgment challenge' },
  { q: 'Why did God make vessels of honor and dishonor?',
    cls: 'Theological',  expected: LOAD,    note: 'Romans 9 vessels challenge' },
  { q: 'Does the Bible teach Universalism if the broad road leads to destruction?',
    cls: 'Theological',  expected: LOAD,    note: 'Universalism and final consequence' },
...
const moduleChecks = [
  ...
  ['Module: foreknown decoherence', PRISM_THEODICY_MODULE.includes('agents who will operate in decoherence')],
  ['Module: Romans 9 vessels',      PRISM_THEODICY_MODULE.includes('vessels for honor and dishonor')],
  ['Module: contextual judgment',   PRISM_THEODICY_MODULE.includes('Romans 2:12–16')],
