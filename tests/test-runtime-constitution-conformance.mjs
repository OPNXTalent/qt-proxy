...
assert.ok(api.includes("timing('artifact_json_repair_start'"));
assert.ok(api.includes('Repair serialization only. Convert the candidate below into the exact JSON contract'));
assert.ok(api.includes("timing('artifact_json_repair_complete'"));
assert.ok(api.includes("structuredOutputName: 'emit_interpretation_artifact'"));
assert.ok(api.includes('structuredOutputSchema: PRISM_ARTIFACT_CORE_SCHEMA'));
assert.ok(api.includes("tool_choice: { type: 'tool', name: structuredOutputName }"));
assert.ok(api.includes("throw new Error('INQUIRY_MODEL_STRUCTURED_OUTPUT_MISSING')"));
assert.ok(api.includes("boundary: 'forced_tool_schema'"));
assert.ok(api.includes("timing('artifact_structured_output_diagnostic'"));
assert.ok(api.includes("outcome: 'provider_rejection'"));
assert.ok(api.includes("outcome: 'tool_use_missing'"));
assert.ok(api.includes('const auditReturnedMeta'));
assert.ok(api.includes('auditReturnedMeta ? artifact.canonicalResponse : auditedCanonical'));
assert.ok(api.includes('metaFallback: auditReturnedMeta'));
const auditMetaPattern = /(?:^|\n)\s*(?:[#>*_`~-]+\s*)*Audit Result\b/i;
assert.ok(auditMetaPattern.test('**AUDIT RESULT: APPROVED**'));
assert.ok(auditMetaPattern.test('### Audit Result: Approved'));
assert.ok(!auditMetaPattern.test('The audit resulted in a revised response.'));
assert.ok(api.includes('Material claims verified'));
assert.ok(api.includes('APPROVED RESPONSE'));
